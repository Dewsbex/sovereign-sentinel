...
class ORBShield:
    ...
    def get_current_equity(self) -> float:
        ...
        try:
            ...
            return float(account.get('total', account.get('totalValue', 0.0)))
    ...
    def execute_kill_protocol(self, session_loss: float, current_equity: float):
        ...
        # Send Telegram alert
        self.send_telegram_alert(session_loss)
        
        # Exit shield
        sys.exit(0)
    
    def send_telegram_alert(self, session_loss: float):
        """Send emergency alert via standardized SovereignAlerts"""
        from telegram_bot import SovereignAlerts
        alerts = SovereignAlerts()
        
        details = (
            f"Session loss exceeded £{self.max_session_loss}\n"
            f"Actual Loss: £{session_loss:,.2f}\n"
            f"Initial equity: £{self.initial_equity:,.2f}\n"
            f"Session positions closed (Strategic holdings protected)."
        )
        
        alerts.send_health_alert("orb_shield.py", "🚨 CIRCUIT BREAKER 🚨", details)
    
    def run(self):
        """
        Main shield loop.
        Runs from 14:25 UTC (pre-flight) until 22:00 UTC (market close).
        """
...
if __name__ == '__main__':
    ...
    # Initialize Logger
    logger = AuditLogger("SS006-ORBShield")
