...
# --- HARD TIME LOCK ---
# US Market Hours (UTC): 14:25 to 22:00 (Extended to 5pm EST)
START = dtime(14, 25)
END = dtime(22, 0)
...
def send_eod_report(alerts: SovereignAlerts, logger: AuditLogger):
    ...
    try:
        ...
        try:
            ...
            current_equity = float(acct.get('total', acct.get('totalValue', 0.0)))
        ...
        msg = (f"MARKET CLOSE (22:00 UTC / 5:00 PM EST)\n"
...
def run_sniper():
    ...
    """
    # Initialize Logger
    logger = AuditLogger("SS009-MainBot")
    ...
    while True:
        try:
            ...
            if now_time > END:
                ...
                if not report_sent:
                    logger.log("SESSION_END", "System", "Past 22:00 UTC cutoff")
            ...
            # 2. FORCE SESSION CLOSE (22:00 UTC - The Curfew)
            # (Run regardless of vacation to clear session trades)
            if now_time >= dtime(22, 0):
                print("22:00 UTC CURFEW: Closing SESSION positions.")
                try:
                    ...
                    if positions:
                        for pos in positions:
                            ...
                            # Ensure positions is a list, API might return a dict on failure
                            if not isinstance(positions, list):
                                logger.log("CURFEW_ERROR", "System", f"Failed to fetch positions: {positions}", "ERROR")
                                sleep_time.sleep(60)
                                continue
                            ...
                            if session_manager.is_whitelisted(ticker):
                                ...
                                session_manager.record_purchase(ticker, qty, 0.0, side="SELL") # Price fallback to 0 for curfew
                            else:
                ...
                if triggers:
                    ...
                    for trade in triggers:
                        try:
                            ...
                            # 3. VWAP Gate (Advisory - ORB breakout confirmation takes priority)
                            if not auditor.check_vwap_gate(ticker, trade['price']):
                                logger.log("VWAP_GATE", ticker, "ADVISORY: Price < VWAP - proceeding with ORB breakout", "INFO")
                                # Note: Not blocking - 5m candle confirm already validates direction
                            ...
                            acct = client.get_account_info()
                            # T212 API v0 returns 'total' for account value
                            total_wealth = float(acct.get('total', acct.get('totalValue', 0.0)))
                            ...
                            qty = max(0.01, max_pos_value / trade['price'])
                            qty = round(qty, 4) # T212 Fractional Precision
                            ...
                            session_manager.record_purchase(ticker, qty, trade['price'], side="BUY")
