...
class SectorMapper:
    ...
    def get_ticker_sector(self, ticker: str) -> str:
        ...
        # specialized normalization for T212 tickers like "AAPL_US_EQ" -> "AAPL"
        
        # Check cache first (ignore if it's 'Unknown' to allow retry after fix)
        if ticker in self.sector_map and self.sector_map[ticker] != "Unknown":
            return self.sector_map[ticker]
            
        # Fetch from yfinance
        try:
            # v2.1: Resolve clean ticker for yfinance compatibility (removes _US_EQ etc)
            _, yf_ticker, inst_data = self.client.resolve_ticker(ticker)
            if not yf_ticker: yf_ticker = ticker
            
            import yfinance as yf
            t = yf.Ticker(yf_ticker)
            sector = t.info.get('sector', 'Unknown')
            
            # Fallback: If yfinance is empty/failing, use Master Universe sector
            if (sector == "Unknown" or not sector) and inst_data:
                m_sector = inst_data.get('sector', 'Unknown')
                # Heuristic mapping for Master Universe categories to GICS
                if any(x in m_sector for x in ['Semi', 'Software', 'AI', 'Cloud', 'Cyber', 'SaaS', 'Data']):
                    sector = "Technology"
                elif any(x in m_sector for x in ['Crypto', 'Payment', 'Bank', 'Broker', 'Exchange']):
                    sector = "Financials"
                elif any(x in m_sector for x in ['Pharma', 'Biotech', 'Genomics', 'mRNA']):
                    sector = "Healthcare"
                elif any(x in m_sector for x in ['EV', 'E-Comm', 'Gaming', 'Travel', 'Metaverse']):
                    sector = "Consumer Discretionary"
                elif any(x in m_sector for x in ['Solar', 'Hydrogen', 'Clean']):
                    sector = "Energy"
                else:
                    sector = m_sector # Use as is if no better mapping

            # Normalize common sector names if needed
